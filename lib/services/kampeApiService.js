...
export class KampeAPIService {
  ...
  async apiCall(method, url, data = null, loadingMessage = "Processing...") {
    try {
      const config = { headers: this.headers, timeout: 60000 };
      ...
      console.error(`Kampe API Error: ${error.message}`);
      return {
        success: false,
  ...
  async getLGAs(stateId) {
    return this.apiCall(
      "GET",
      `${KAMPE_BASE_URL}/v1/auth/lga/${stateId}`,
      null,
  ...
  async createEnrollment(enrollmentData) {
    return this.apiCall(
      "POST",
      `${KAMPE_BASE_URL}/v1/auth/register`,
      enrollmentData,
      "Processing registration"
    );
  }

  // Generate invoice 🚨🚨🚨🚨🚨🚨🚨 YET TO BE IMPLEMENTED
