...
export class MessageRouter {
  static async route(userId, text) {
    ...
    // Check if server was recently restarted and user has no existing session
    if (SessionManager.isServerRecentlyRestarted() && !SessionManager.hasExistingSession(userId) && !isGreeting(text)) {
      return {
        message: SessionManager.getRestartMessage(),
        state: STATES.MAIN_MENU,
      };
    }
