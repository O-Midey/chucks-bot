...
import { HealthInsuranceHandler } from "./handlers/healthInsuranceHandler/healthInsuranceHandler.js";
...
export class MessageRouter {
  static async route(userId, text) {
    ...
    const timeoutCheck = await SessionManager.checkTimeout(userId);
    ...
    if (isGreeting(text)) {
      // Inform user their previous session (if any) was cleared, then show welcome
      const clearedMsg = await SessionManager.clearSession(userId, true);
      const welcome = MessageTemplates.getWelcome();
      return {
        message: (clearedMsg ? clearedMsg + "\n\n" : "") + welcome,
        state: STATES.MAIN_MENU,
      };
    }
