...
import { HealthInsuranceHandler } from "./handlers/healthInsuranceHandler.js";
...
export class MessageRouter {
  static async route(userId, text) {
    ...
    if (SessionManager.checkTimeout(userId)) {
      return {
        message:
          "⏰ Your session has timed out. Let's start fresh!\n\n" +
          MessageTemplates.getWelcome(),
    ...
    try {
      switch (session.state) {
        ...
        case STATES.HEALTH_PLANS_LIST:
          return HealthInsuranceHandler.handlePlanSelection(
            userId,
            input,
            text
          );
        ...
        case STATES.HEALTH_REG_SURNAME:
          return HealthInsuranceHandler.handleSurname(userId, input, text);
        case STATES.HEALTH_REG_MIDDLENAME:
          return HealthInsuranceHandler.handleMiddleName(userId, input, text);
        case STATES.HEALTH_REG_FIRSTNAME:
          return HealthInsuranceHandler.handleFirstName(userId, input, text);
        case STATES.HEALTH_REG_EMAIL:
          return HealthInsuranceHandler.handleEmail(userId, input, text);
        case STATES.HEALTH_REG_PHONE:
          return HealthInsuranceHandler.handlePhone(userId, input, text);

        case STATES.HEALTH_REG_MARITAL:
          return HealthInsuranceHandler.handleMaritalStatus(
            userId,
            input,
            text
          );
        ...
        case STATES.HEALTH_REG_ADDRESS:
          return HealthInsuranceHandler.handleAddress(userId, input, text);

        case STATES.HEALTH_REVIEW:
          return HealthInsuranceHandler.handleReview(userId, input, text);
        case STATES.HEALTH_REVIEW_EDIT:
          return HealthInsuranceHandler.handleReviewEdit(userId, input, text);
        ...
        case STATES.PAYMENT_CONFIRMATION:
          return await HealthInsuranceHandler.handlePaymentConfirmation(userId, input, text);
