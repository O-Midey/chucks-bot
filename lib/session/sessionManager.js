...
const restartMessageShown = new Set(); // Track users who've seen restart message
...
export class SessionManager {
  ...
  static hasExistingSession(userId) {
    return sessions.has(userId);
  }

  static isServerRecentlyRestarted() {
    return Date.now() - serverStartTime < 5000; // Only 5 seconds
  }

  static shouldShowRestartMessage(userId) {
    return this.isServerRecentlyRestarted() && !restartMessageShown.has(userId);
  }

  static markRestartMessageShown(userId) {
    restartMessageShown.add(userId);
  }
