...
export class HealthInsuranceHandler {
  ...
  // ============================================
    ...
    static async processEnrollment(userId) {
      ...
      try {
        ...
        // Redirect to PaymentHandler to initiate GlobalPay payment

        return PaymentHandler.getPaymentOptions(userId, data.planPremium);
  ...
  static async initiateKampeEnrollment(userId) {
    ...
    // Remove the kampeEnrollmentData from session once we start processing
    SessionManager.updateSession(userId, STATES.HEALTH_PROCESSING, {
        kampeEnrollmentData: null // Clear this specific data
    });

    if (!kampeEnrollmentData) {
      console.error("Kampe Enrollment Data not found in session for userId:", userId);
      return {
        message: `An error occurred. Enrollment data was not found. Please start over from the main menu.`,
        state: STATES.MAIN_MENU,
      };
    }
