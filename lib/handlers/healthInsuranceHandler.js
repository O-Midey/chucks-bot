...
export class HealthInsuranceHandler {
  ...
  static async handleGlobalCommands(userId, input, currentState) {
    if (input === "back" || input === "previous") {
      return await this.goBackOneStep(userId, currentState);
    }
    return null; // No global command matched
  }

  static async goBackOneStep(userId, currentState) {
    const stateFlow = {
    ...
    if (previousState) {
      ...
      if (previousState === STATES.HEALTH_PLANS_LIST) {
        return await this.showPlans(userId);
      }
      const prompt = this.getPromptForState(previousState);
  ...
  static async handlePlanDetails(userId, input, text) {
    ...
    return await this.handlePlanSelection(userId, input, text);
  }
  ...
  static async handleReviewEdit(userId, input, text) {
    ...
    if (!nextAction) {
      if (input === "5" || input.includes("cancel")) {
        return StateManager.updateAndReturn(
          ...
          this.showReviewSummary(userId).message
        );
      }
      return {
        message: `Please select a valid option (1-5).`,
    ...
    // Handle Personal Information submenu
  ...
  static async handlePersonalEdit(userId, input, text) {
    ...
    if (!nextState) {
      ...
      return {
        ...
        state: STATES.HEALTH_PERSONAL_EDIT,
      };
    }

    const messages = {
      [STATES.HEALTH_REG_SURNAME]: "What is your *Surname* (Last Name)?",
      [STATES.HEALTH_REG_MIDDLENAME]: 'What is your *Middle Name*?\n\n_Type "SKIP" if you don\'t have a middle name._',
      [STATES.HEALTH_REG_FIRSTNAME]: "What is your *First Name*?",
      [STATES.HEALTH_REG_EMAIL]: "What is your *Email Address*?\n\n_Please enter a valid email address_\n_Example: john@example.com_",
      [STATES.HEALTH_REG_PHONE]: "What is your *Phone Number*?\n\n_Please enter your 11-digit Nigerian phone number_\n_Example: 08012345678_",
    };
