...
export class HealthInsuranceHandler {
  ...
  static async showProviders(userId) {
    try {
      ...
      return {
        message: `An error occurred while loading providers. Please try again or type MENU.`,
        state: STATES.MAIN_MENU,
      };
    }
  }
  ...
  static async showPlans(userId) {
    try {
      ...
      // Parse plans (adjust based on actual API response)
      let plansArray = [];
  ...
  static async handlePlanSelection(userId, input, text) {
    ...
    if (planNumber >= 1 && planNumber <= plans.length) {
      ...
      SessionManager.updateSession(userId, STATES.HEALTH_REG_SURNAME, {
        ...
        planPremium: selectedPlan.premium || selectedPlan.price,
      });
  ...
  static async fetchAndShowLGAs(userId, stateId) {
    try {
      ...
      console.log("LGAs RAW DATA:", lgasArray);
  ...
  static async handleAddress(userId, input, text) {
    ...
    const result = await this.showProviders(userId);
    return result;
  }
  ...
  static async processEnrollment(userId) {
    ...
    try {
      ...
      // Create enrollment via Kampe API
      ...
      if (enrollmentResult.message !== "Successfully created user!") {
        return {
          message: `Sorry, there was an error processing your enrollment. Please try again or contact support.\n\nError: ${enrollmentResult.message || enrollmentResult.error}`,
          state: STATES.HEALTH_REVIEW,
        };
      }

      // For now, just show success message since enrollment worked
      SessionManager.updateSession(userId, STATES.MAIN_MENU);

      let message = `✅ *Registration Successful!*\n\n`;
      message += `🎉 Your health insurance registration has been completed successfully!\n\n`;
      message += `📋 *Registration Details:*\n`;
      message += `Name: ${data.firstname} ${data.middlename} ${data.surname}\n`;
      message += `Provider: ${data.providerName}\n`;
      message += `Location: ${data.lga}, ${data.state}\n\n`;
      message += `You will be contacted shortly for next steps.\n\n`;
      ...
      return {
        message: `An error occurred during enrollment. Please try again later or contact support.`,
        state: STATES.MAIN_MENU,
      };
    }
  }
  ...
  static async handlePayment(userId, input, text) {
    if (
      input.includes("done") ||
      input.includes("paid") ||
      input.includes("completed")
    ) {
      SessionManager.updateSession(userId, STATES.HEALTH_PAYMENT_VERIFY);

      let message = `⏳ *Verifying your payment...*\n\n`;
      message += `Please wait while we confirm your payment.`;

      // Send verification message first
      setTimeout(() => {
        this.verifyAndActivatePolicy(userId);
      }, 2000);

      return {
        message,
        state: STATES.HEALTH_PAYMENT_VERIFY,
      };
    }
  ...
  static handlePolicyActivated(userId, input, text) {
    const session = SessionManager.getSession(userId);
    const data = session.data;

    if (input === "1" || input.includes("benefit")) {
      let message = `📋 *Your Policy Benefits*\n\n`;
      message += `Policy Number: ${data.policyNumber}\n`;
      message += `Plan: ${data.planName}\n\n`;
      message += `*Coverage Includes:*\n`;
      message += `• Hospital bills coverage\n`;
      message += `• Outpatient consultations\n`;
      message += `• Prescription medications\n`;
      message += `• Diagnostic tests & scans\n`;
      message += `• Emergency services\n`;
      message += `• Specialist consultations\n\n`;
      message += `*Your Preferred Hospital:*\n`;
      message += `${data.hospital}\n`;
      if (data.hospitalAddress) {
        message += `${data.hospitalAddress}\n`;
      }
    ...
    if (
      ...
      let message = `📄 *Policy Certificate*\n\n`;
      message += `Your policy certificate is being generated...\n\n`;
      message += `Download link:\nhttps://portal.yourcompany.com/certificates/${data.policyNumber}\n\n`;
      message += `You will also receive it via email at: ${data.email}\n\n`;
      message += `Type MENU to return to main menu.`;
    ...
    if (input === "3" || input.includes("support")) {
      let message = `🛟 *Customer Support*\n\n`;
      message += `Need help? We're here for you!\n\n`;
      message += `📞 Call: +234 XXX XXX XXXX\n`;
      message += `📧 Email: support@yourcompany.com\n`;
      message += `💬 WhatsApp: This number\n\n`;
      message += `Or type AGENT to speak with a live representative.\n\n`;
      message += `Type MENU to return to main menu.`;
    ...
    if (input === "4" || input.includes("menu")) {
      SessionManager.clearSession(userId);
      return {
    ...
    return {
      message: `Please select an option (1-4).`,
      state: STATES.HEALTH_POLICY_ACTIVATED,
    };
  }
}
