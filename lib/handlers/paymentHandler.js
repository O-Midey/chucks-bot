...
export class PaymentHandler {
  static getPaymentOptions(userId, premium) {
    // store the premium in the session so it can be accessed in handleMethod
...
_Select your preferred payment method._`,
  ...
  static async handleMethod(userId, input, text) {
    if (input === "1" || input.includes("online") || input.includes("card")) {
      ...
      const premium = session.data.premium;
      ...
      if (missingFields.length > 0) {
        return {
          ...
          state: STATES.MAIN_MENU, // Or any other state to collect missing info
        };
      }
...
❌ Cancel`;
            ...
            const message =
              "An unknown error occurred with the payment gateway. Please try again or contact support.";
...
After payment, reply "DONE"`,
  ...
  static async handleConfirmation(userId, input, text) {
    if (
      input.includes("paid") ||
      input.includes("done") ||
      ...
      if (session.data.kampeEnrollmentData) {
        ...
        await SessionManager.updateSession(
          userId,
          STATES.HEALTH_INITIATE_ENROLLMENT,
          {
