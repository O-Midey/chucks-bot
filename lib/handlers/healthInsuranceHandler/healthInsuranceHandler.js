...
export class HealthInsuranceHandler {
  ...
  static validateSession(userId, expectedState) {
    const session = SessionManager.getSession(userId);
    
    // Check if session was reset to MAIN_MENU but we're expecting a different state
    if (session.state === STATES.MAIN_MENU && expectedState !== STATES.MAIN_MENU) {
      // Only show expired message if this is NOT a fresh session
      if (!session.isNew && (!session.data || Object.keys(session.data).length === 0)) {
        return {
          isValid: false,
  ...
  static async handle(userId, input, text, currentState) {
    ...
    const validation = this.validateSession(userId, currentState);
...
handlers/
  healthInsuranceHandler/
    ├── HealthInsuranceHandler.js (main handler - this exports the class)
    ├── config/
    │   └── healthPlans.js
    ├── validators/
    │   └── HealthInsuranceValidator.js
    ├── formatters/
    │   └── HealthInsuranceFormatter.js
    ├── steps/
    │   ├── BaseStep.js
    │   ├── PlanSelectionStep.js
    │   ├── PersonalInfoStep.js
    │   ├── LocationStep.js
    │   ├── ProviderSelectionStep.js
    │   ├── ReviewStep.js
    │   └── PaymentStep.js
    └── navigation/
...
1. **Separation of Concerns**: Each step is now its own class
2. **Single Responsibility**: Each file has one clear purpose
3. **Easier Testing**: Can test individual steps in isolation
4. **Better Maintainability**: Changes to one step don't affect others
5. **Cleaner Code**: Reduced duplication and better organization
6. **Scalability**: Easy to add new steps or modify existing ones
7. **Reusability**: Validators and formatters can be used elsewhere

MIGRATION STRATEGY:

1. Keep the old HealthInsuranceHandler.js as a backup
2. Create the new folder structure
3. Copy each section into its respective file
4. Update imports in messageRouter.js to use the new handler
5. Test thoroughly
6. Remove the old file once confirmed working

The main handler now acts as a router, delegating to appropriate step handlers.
All backward compatibility methods are included so existing code continues to work.
*/
